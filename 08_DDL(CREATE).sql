...
-- 문자인코딩 UTF-8 : 영어, 숫자 1BYTE, 한글, 특수문자등은 3BYTE
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER" VALUES('MEM02', 'QWER1234', '김영희','960606-2121212',SYSDATE);


-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO "MEMBER" VALUES('MEM03', 'JJJJJA', '박철수','921212-1212121', DEFAULT);
...
VALUES ('MEM04', 'PASSWORD', '이지연');

-- ** JDBC에서 날짜를 입력 받았을 때 삽입하는 방법 **
-- '2022-09-13 17:33:27'
INSERT INTO "MEMBER" VALUES ('MEM05', 'PASSS', '김길동', '930303-1030303', 
...
-- 제일 앞에 0이 있으면 이를 자도으로 제거함
--> 전화번호, 주민등록번호처럼 숫자로만 되어있는 데이터지만
-- 0으로 시작할 가능성이 있으면 CHAR, VARCHAR2 문자형 사용
...
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
--    USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨 (제약 조건명 미지정) - 자동으로 번호 부여
...
--  CHECK는 작성후 수정이 번거롭지만 FK는 부모 테이블에 행을 추가/수정/삭제하는게 간편
...
--> 서브쿼리로 테이블 생성시 
-- 테이블의 형태(컬럼명, 데이터 타입) + NOT NULL 제약 조건만 복사되고
...
ALTER TABLE EMPLOYEE ADD CONSTRAINT EMP_SAL_LECEL 
