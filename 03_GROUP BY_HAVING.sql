...
-- EMPLOYEE 테이블에서
-- 성별(남, 여)과 각 성별 인원 수, 급여 합을
-- 인원 수 오름차순으로 조회

SELECT DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여'  ) 성별,
	COUNT(*) "인원 수", SUM(SALARY) " 급여 합"
FROM EMPLOYEE
GROUP BY DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여'  ) -- 별칭 사용 X(별칭 해석 X) 
...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- DEPT_CODE로 그룹을 나누고 , 나눠진 그룹내에서 
                             -- JOB_CODE로 또 그룹을 분류 -> 세분화
...
FROM EMPLOYEE
--WHERE SALARY >=3000000 --> 한 사람의 급여가 300만 이상이라는 조건 (요구사항 충족 X)
GROUP BY DEPT_CODE 
HAVING AVG(SALARY) >=3000000  --> DEPT_CODE  그룹 중 급여 평균이 3백만 이상인 그룹 조회
ORDER BY DEPT_CODE;
...
FROM EMPLOYEE
GROUP BY JOB_CODE 
HAVING COUNT(*) <=5  -- HAVING절에는 그룹함수가 반드시 들어간다
ORDER BY JOB_CODE; 
