...
-- 함수는 SELECT문의
-- SELECT절, WHERE절, ORDER BY절, GROUP PY절, HAVING절 사용 가능
...
SELECT ROUND(123.456, 0) FROM DUAL; -- 소수점 첫번째 자리에서 반올림 (0이 기본값)

SELECT ROUND(123.456, -1) FROM DUAL; -- 소수점 -1 자리에서 반올림
                                     -- 1의 자리에서 반올림해서 10의 자리부터 표현


SELECT ROUND(123.456, -2) FROM DUAL; -- 10의 자리에서 반올림 100의 자리부터 표현 
...
SELECT ROUND(MONTHS_BETWEEN(SYSDATE, '2022-08-04'), 3) "수강 기간" FROM DUAL; 
-- 0.9174757318 -> 1개월 단위 
...
-- R 패턴 : 1세기를 기준으로 절반(50) 이상인 경우 이전 세기(1900년대)
...
-- 급여 최대값, 가장 최근 입사일, 이메일 알파벳 순서가 가장 느린 이메일
