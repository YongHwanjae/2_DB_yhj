...
-- kh_yhj / yhj_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- 1. (sample) kh_yhj 계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh_yhj.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 접근 권한이 없어서 조회가 불가능


-- 2. (kh) yhj_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO yhj_sample;

-- 3.(sample) 다시 kh_yhj.EMPLOYEE 조회
SELECT * FROM kh_yhj.EMPLOYEE;
...
-- 4. (sample) kh_yhj.EMPLOYEE 테이블을 복사한 테이블을 생성
CREATE TABLE EMP_SAMPLE 
AS SELECT * FROM kh_yhj.EMPLOYEE;
